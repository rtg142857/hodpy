...
def get_luminosity_function(path_config_filename):
    '''
    ...
    Args:
        path_config_filename: Location of the path_config yaml file
    '''
    ...
    hod = HOD_BGS(path_config_filename, redshift_evolution=False, #photsys=photsys, mag_faint=mag_faint, mag_faint=type="absolute",
    ...
    output_file = lookup.bgs_lf_target.format(label)
...
if __name__ == '__main__':
    ...
    get_luminosity_function(path_config_filename=path_config_filename)
    ...
    hod = HOD_BGS(path_config_filename, redshift_evolution=True, mag_faint_type="absolute", mag_faint=-18,
                replace_central_lookup=True, replace_satellite_lookup=True,
                replace_slide_file=True)
   
    # Use the HOD to calculate the fraction of galaxies that are centrals
    # Again, it doesn't matter here what photsys is set to
    col = ColourDESI(path_config_filename, photsys='S', hod=hod, replace_central_fraction_lookup_file=True)
