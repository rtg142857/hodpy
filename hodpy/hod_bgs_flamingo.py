...
from hodpy.mass_function import MassFunctionFlamingo
...
class HOD_BGS(HOD):
    ...
    args:
        path_config_filename: path to the path_config.yml file containing config info
        NO COSMO
        NO PHOTSYS
    ...
    def __init__(self, path_config_filename, #cosmo, photsys, mag_faint_type='apparent', mag_faint=20.2, mag_faint_extrapolate=-17, z0=0.2,
        ...
        #self.c = cosmo
        self.cosmo = CosmologyFlamingo(path_config_filename)
        ...
        hod_parameters = lookup.read_hod_param_file_flamingo(hod_param_file.format(label))
        ...
        if self.redshift_evolution:
            if slide_file is None:
                ...
                slide_file = lookup.flamingo_hod_slide_factors.format(label)
        ...
        if satellite_lookup_file is None:
            satellite_lookup_file = lookup.satellite_lookup_file.format(label,photsys)
            print("Making satellite lookup file...")
